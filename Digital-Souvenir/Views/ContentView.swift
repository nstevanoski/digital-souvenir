        ...
        VStack{
            if self.status{
                TabView {
                    ...
                    SearchView().tabItem {
                        Image(systemName: "magnifyingglass")}.tag(3).environmentObject(productVM)
                    CollectionView().tabItem {
                        Image(systemName: "heart").tag(4)
                    }
                    ProfileView().tabItem {
                        Image(systemName: "person.fill")}.tag(5).environmentObject(userVM).environmentObject(orderVM).environmentObject(productVM)
